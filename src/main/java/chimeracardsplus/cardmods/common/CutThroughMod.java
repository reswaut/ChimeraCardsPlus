...
public class CutThroughMod extends AbstractAugmentPlus implements DynvarCarrier {
    ...
    public boolean validCard(AbstractCard abstractCard) {
        return cardCheck(abstractCard, c -> c.cost >= -1
                ...
                && (abstractCard.baseDamage >= 2 || abstractCard.baseBlock >= 2 || abstractCard.baseMagicNumber >= 2 && doesntDowngradeMagic()));
    }
    ...
    public float modifyBaseMagic(float magic, AbstractCard card) {
        if (Stream.of(CutThroughFate.ID, JustLucky.ID, ThirdEye.ID).anyMatch(s -> s.equals(card.cardID))) {
            return magic * 0.75F + baseVal(card);
        }
    ...
    public void onUse(AbstractCard card, AbstractCreature target, UseCardAction action) {
        if (Stream.of(CutThroughFate.ID, JustLucky.ID, ThirdEye.ID).noneMatch(s -> s.equals(card.cardID))) {
            addToBot(new ScryAction(baseVal(card)));
        }
