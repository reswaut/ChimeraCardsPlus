...
public class SuperluminalMod extends AbstractAugment implements DynvarCarrier {
    ...
    public float modifyBaseMagic(float magic, AbstractCard card) {
        return (magic >= 1.0F && doesntDowngradeMagicNoUseChecks(card)) ? Math.max(magic - 3.0F, 1.0F) : magic;
    }

    @Override
    public boolean validCard(AbstractCard card) {
        return cardCheck(card, c -> c.cost != -2) &&
                (card.baseDamage >= 4 || card.baseBlock >= 4 || (card.baseMagicNumber >= 4 && doesntDowngradeMagicNoUseChecks(card)));
    }
